...
class CreatePostResponse(BaseModel):
    ...
    post: ApiPost
    message: StrictStr
    __properties: ClassVar[List[str]] = ["post", "message"]

    @field_validator('message')
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Post published successfully', 'Post scheduled successfully', 'Draft created successfully']):
            raise ValueError("must be one of enum values ('Post published successfully', 'Post scheduled successfully', 'Draft created successfully')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "post": ApiPost.from_dict(obj["post"]) if obj.get("post") is not None else None,
